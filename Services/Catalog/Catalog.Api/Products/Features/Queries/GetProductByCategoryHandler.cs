...
{
    public async Task<Result<GetProductByCategoryResponse>> Handle(GetProductByCategoryQuery request, CancellationToken cancellationToken)
    {
        ...
        return Result.Ok(new GetProductByCategoryResponse(results.Select(p => new ProductDto(p))));
    }
}
